  ...
  try {
    ...
    try {
      ...
      if (session.status !== 'complete') {
        ...
        return new Response(
          JSON.stringify({ verified: false, error: "Payment not complete" }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
            status: 400,
          }
      ...
      if (userId && session.client_reference_id !== userId) {
        ...
        return new Response(
          JSON.stringify({ verified: false, error: "User ID mismatch" }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
            status: 400,
          }
      ...
      return new Response(
        JSON.stringify({ verified: false, error: `Invalid session: ${stripeError.message}` }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 400,
        }
