...
const AddMemoryPage = () => {
  ...
  // CRITICAL: Function to handle Stripe checkout
  const openPaymentModal = async () => {
    // Step 1: Log entry and set loading state
    console.log("DEBUG: Frontend - openPaymentModal triggered. (Frontend Part 2.2)"); // DEBUG LOG
    setIsSubmitting(true); // Show loading/saving state on the button/UI

    // Step 2: Verify user authentication
    const user = supabase.auth.currentUser; // Get current user from AuthContext
    if (!user || !user.id) {
        console.error("DEBUG: Frontend - Cannot open payment modal: User not logged in. (Frontend Part 2.2)");
    ...
    try {
        // CRITICAL: Call your deployed create-checkout-session Edge Function
        console.log("DEBUG: Frontend - Calling create-checkout-session Edge Function. (Frontend Part 2.2)"); // DEBUG LOG
        const response = await fetch('https://oeccgchvvewljcrfayrg.supabase.co/functions/v1/create-checkout-session', { // CRITICAL: Use your EXACT Edge Function URL
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                userId: user.id, // Pass the Supabase user ID
                priceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_ID, // CRITICAL: Get Price ID from frontend env var
                successUrl: `${window.location.origin}/#/home`, // Redirect back to home on success
                cancelUrl: `${window.location.origin}/#/home` // Redirect back to home on cancel
            })
        });
        ...
        // CRITICAL: Redirect to Stripe Checkout
        console.log("DEBUG: Frontend - Redirecting to Stripe Checkout with sessionId:", sessionId, "(Frontend Part 2.2)"); // DEBUG LOG
        const stripe = await loadStripe(process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY); // Load Stripe.js with publishable key
        await stripe.redirectToCheckout({ sessionId: sessionId });

    } catch (e) {
        console.error("DEBUG: Frontend - Error opening Stripe Checkout:", e.message, "(Frontend Part 2.2)"); // DEBUG LOG
        // Display error to user (e.g., "Failed to open payment. Please try again.")
    } finally {
        setIsSubmitting(false); // Hide loading state
        console.log("DEBUG: Frontend - openPaymentModal finished. (Frontend Part 2.2)"); // DEBUG LOG
    }
  };
  ...
  const handleDateSelect = async (day, month) => {
    ...
    try {
      ...
      if (user?.is_premium) {
        console.log("DEBUG: User is premium. Granting unlimited memory access. (Frontend Part 2.3)");
        // Premium users bypass the memory limit check
      } else {
        // User is NOT premium, so check free trial limit
        console.log("DEBUG: User is NOT premium. Checking free trial limit. (Frontend Part 2.3)");
        ...
        if (countError) {
          console.error("DEBUG: Error fetching memory count:", countError.message, "(Frontend Part 2.3)");
          setError("Failed to check memory limit. Please try again.");
          setIsSubmitting(false);
          return;
        }
        
        console.log("DEBUG: Current memories for user:", count, "(Frontend Part 2.3)");
        if (count >= FREE_MEMORY_LIMIT) {
          console.log("DEBUG: Free trial limit reached. Triggering payment pop-up. (Frontend Part 2.3)");
          setError("Free trial limit reached. Please upgrade to add more memories!");
          setIsSubmitting(false);
          
          // CRITICAL: Call the function that opens the Stripe payment modal
          openPaymentModal(); // <--- This function must be called here
          return; // Stop memory insertion
        }
      }
      ...
      if (error) {
        console.error("DEBUG: Error saving memory:", error.message, "(Frontend Part 2.3)");
        setError(error.message || 'Failed to save memory');
        return;
      }

      console.log("DEBUG: Memory saved successfully:", data, "(Frontend Part 2.3)");
      ...
      console.error("DEBUG: Unexpected error during memory save:", err.message, "(Frontend Part 2.3)");
...
console.log("DEBUG: Free trial message displays premium status. (Frontend Part 2.2)");
console.log("DEBUG: Memory limit check logic updated to use is_premium flag. (Frontend Part 2.3)");
